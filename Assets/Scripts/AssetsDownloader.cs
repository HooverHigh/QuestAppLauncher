...
namespace QuestAppLauncher {
  ...
  public class AssetsDownloader {
    ...
    // APK Remote config url
    const string APKCUrl = @ "http://vrdevices.hooverhigh.ml/sys/oculus-apk.json";
    ...
    private async Task < Dictionary < string, AssetInfo >> DownloadAssetsMetadata(
      ...
      var assetsInfo = new Dictionary < string,
        AssetInfo > (StringComparer.OrdinalIgnoreCase);
        ...
        using(var req = new UnityWebRequest(requestUrl)) {
          ...
          foreach(var property in jObject["assets"]) {
            ...
            if ((name.StartsWith("iconpack") && name.EndsWith(".zip")) ||
              ...
              assetsInfo[name] = new AssetInfo {
                RepoUri = repoUri, Url = url, UpdatedAt = updatedAt, TagName = tagName
              };
            }
          }
        }
